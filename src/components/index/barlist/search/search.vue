  ...
  <div id='search'>
    <div class="serach_bar" :style='serachBar'>
      <div class="search">
        ...
        <input @click='songInput' @keyup.enter='serachEnter' @keyup.delete='inputDelete' v-model='searchData'
               placeholder="搜索歌曲、歌单、专辑" class='serach_input' type="text" name="search">
    ...
    <div class="remen_search" v-show='!cancel'>
      ...
      <div class="remen_list">
        <span class="remen_list_item tag_host" @click='setSearchValue(special_key)'
              :title='special_url'>{{special_key}}</span>
    ...
    <div class='songList' v-if='songListShow'>
      <div class="bscroll" ref="bscroll">
        <div v-if='developer' class="developer_person">
          <span>开发者-frank-vue首次</span>
        </div>
        <ul>
          <li class='songList_li' v-if='querySearchData.zhida.type == 2'
              @click='gotoSinger(querySearchData.zhida.singermid,querySearchData.zhida.type)'>
            <img class='geshou_fm left' v-if='querySearchData.zhida.singermid && querySearchData.zhida.singermid != ""'
                 :src="'https://y.gtimg.cn/music/photo_new/T001R68x68M000'+querySearchData.zhida.singermid+'.jpg?max_age=2592000'"
                 alt="">
          ...
          <li class='songList_li' v-if='querySearchData.zhida.type == 3'
              @click='gotoSinger(querySearchData.zhida.albummid,querySearchData.zhida.type)'>
            <img class='geshou_fm left' v-if='querySearchData.zhida.albummid && querySearchData.zhida.albummid != ""'
                 :src="'https://y.gtimg.cn/music/photo_new/T002R68x68M000'+querySearchData.zhida.albummid+'.jpg?max_age=2592000'"
                 alt="">
            <div class="geshou_info left">
          ...
          <!-- 由于qq音乐传过来的id重复了 所以此处只能使用下标(index)  此处只是获取数据  没有影响  --作者:rohlin   -->
          <li v-for='(songDataList,index) of querySearchData.song.list'
              @click='gotoplaysong(songDataList.pay.paydownload == 0 && songDataList.pay.payinfo == 0 && songDataList.pay.paytrackmouth == 0 && songDataList.t == 0 && songDataList.alertid == 0,songDataList.songid,songDataList.songmid,songDataList.pay.payplay == 1)'
              :key='index' class='songList_li'>
            <i class="iconfont qq_music">&#xe680;</i>
            <div class="geshou_info left">
              <h4 v-html='songDataList.songname'></h4>
              <div class="geshou_other">
                <span v-for='(songItem,index) of songDataList.singer' :key='songItem.id' v-html='songItem.name'><i
                  v-if='index>0'>/</i></span>
              </div>
            </div>
          </li>
    ...
    <!-- 搜索历史 -->
    <div class="rohlin_search_history" v-show='searchHistoryFlag'>
      <ul class="search_history_ul">
        <li class="clear" v-for='(searchHistoryItem,index) of searchHistory' :key='index'
            @click='clickHistory(searchHistoryItem)'><i class="iconfont history_icon left">&#xe74d;</i><span
          class="left history_text">{{searchHistoryItem}}</span><i class="clear_his_icon iconfont right"
                                                                   @click.stop='clearHistory(searchHistoryItem)'>&#xe646;</i>
        </li>
      </ul>
...
export default {
  ...
  methods: {
    ...
    gotoplaysong (flag, songid, songmid, payplay) {
      let that = this
      if (flag) {
        that.$MessageBox('高仿QQ音乐', '抱歉,此歌不允许播放！')
      } else if (payplay) {
        that.$MessageBox('高仿QQ音乐', '唱片公司不让免费听这首歌了，可前往qq音乐客户端购买后畅享')
      } else {
        that.$router.push({
          path: '/playsong',
          query: {
