<template>
  <div id="mv">
    <video :poster="getMVInfo.cover_pic_medium!=undefined?getMVInfo.cover_pic_medium:''" controls
           :src="getMVUrl != ''?getMVUrl:''" width='100%'></video>
    ...
    <div class="details" v-show="toogle == 0">
      ...
      <ul class="video_list" v-if="getRecomMV != null && getRecomMV != undefined">
        <li v-for="(getRecomMVItem,index) of getRecomMV.list" :key="index"
            @click="gotoCurrentMv(getRecomMVItem.vid,getRecomMVItem.type)">
          ...
          <div class="qui_list">
            ...
            <p class="qui_list__desc">
              ...
              <img class="mv_img" v-if="getRecomMVItem.type == 0"
                   src="//y.gtimg.cn/mediastyle/mobile/project_v5/img/mv/mv_icon_S.png?max_age=2592000">
              <span v-if="getRecomMVItem.type == 1">&nbsp;&nbsp;{{getRecomMVItem.uploader_nick}}</span>
              <span>&nbsp;<i v-for="(singers,index) of getRecomMVItem.singers"
                             :key="index">{{singers.name}}</i>&nbsp;</span>
            </p>
          </div>
        </li>
        ...
        <li v-for="(commentlistitem,index) of mvreplay.hot_comment.commentlist" :key="index">
          <div class="mt_top">
            ...
            <div class="mt_top_left">
              <h4><span>{{commentlistitem.nick}}</span><img v-if='commentlistitem.vipicon != ""' class="vipicon_icon"
                                                            :src="commentlistitem.vipicon" alt=""></h4>
              <time v-html="time(commentlistitem.time)"></time>
            </div>
            <span @click="noclick" class="mt_top_right"><i>{{commentlistitem.praisenum}}</i><i
