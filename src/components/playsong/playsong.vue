<template>
  <div id='playsong'>
    ...
    <div class="main" v-if='data.detail && data.detail != null && data.detail != ""'>
      <div class="bg">
        <img
          :src="'https://y.gtimg.cn/music/photo_new/T002R300x300M000'+data.detail.data.track_info.album.mid+'.jpg?max_age=2592000'"
          alt="">
      </div>
      <div class="content">
        <div class='center_content'>
          <h1 class="song_name">{{data.detail.data.track_info.title}}&nbsp;<i class="iconfont menu_icon"
                                                                              @click="noOpen">&#xe611;</i></h1>
    ...
    <div class="cnxht" v-if='data.simsongs && data.simsongs != null && data.simsongs != ""'>
      ...
      <ul class="cnxh_ul">
        <li class="" @click="clickView" v-for="songInfoList of data.simsongs.data.songInfoList" :key="songInfoList.id">
          <img class="cnxh_img"
               :src="'https://y.gtimg.cn/music/photo_new/T002R150x150M000'+songInfoList.album.mid+'.jpg?max_age=2592000'"
               alt="">
          <div class="name_dec">
    ...
    <div class="rohlin_zuixin_zhuanji"
         v-if='data.gedan && data.gedan != null && data.gedan != "" && data.gedan.data.vec_gedan.length>0'>
      <div class="zuixin_zhuanji">
        <h1 class="rohlinqui_tit__text">大家都在听</h1>
      </div>
      <ul class="zhuanji_ul">
    ...
    <div class="cnxht" v-if='data.detail && data.detail != null && data.detail != ""'>
      ...
      <ul class="cnxh_ul">
        <li @click="clickView" v-for="singer of data.detail.data.track_info.singer">
          <img class="cnxh_img"
               :src="'https://y.gtimg.cn/music/photo_new/T001R150x150M000'+singer.mid+'.jpg?max_age=2592000'" alt="">
          <div class="name_dec">
    ...
    <div class="cnxht" v-if='comment.hot_comment && comment.hot_comment != "" && comment.hot_comment != null'>
      ...
      <div class="mt_top">
        ...
        <div class="mt_top_left">
          <h4><span>{{comment.hot_comment.commentlist[0].nick}}</span><img
            v-if='comment.hot_comment.commentlist[0].vipicon != ""' class="vipicon_icon"
            :src="comment.hot_comment.commentlist[0].vipicon" alt=""></h4>
          <time v-html="time(comment.hot_comment.commentlist[0].time)"></time>
        </div>
        <span @click="clickView" class="mt_top_right"><i>{{comment.hot_comment.commentlist[0].praisenum}}</i><i
    ...
    <div class="cnxht" v-if='data.video && data.video != null && data.video != "" && data.video.data.list.length>0'>
      ...
      <div class="video_con" v-for='(videoItem,index) of data.video.data.list' :key='videoItem.mvid'>
        <div class="video_fm">
          ...
          <span class="cover_play" @click="playVideo($event,videoItem.vid)"
                :style="'background:url('+coverPlay+') top ;background-size:50px;'"></span>
        </div>
        <div class="video_detail">
          <h4>{{videoItem.title}}</h4>
          <p><span v-for='(singers,index) of videoItem.singers'>{{singers.name}}<i
            v-if='videoItem.singers.length > 0 && index<videoItem.singers.length-1'>&nbsp;/&nbsp;</i></span></p>
        </div>
      </div>
    </div>
    ...
    _getmusicu () {
      ...
      getmusicu(that.songid.length > 0 ? that.songid[that.songidIndex] : that.searchSongid, res => {
        ...
        if (res.data.code == ERROR_OK) {
          ...
          console.log('出现异常，请联系开发者：1147296957@qq.com')
        }
      })
    },
    _getreplay () {
